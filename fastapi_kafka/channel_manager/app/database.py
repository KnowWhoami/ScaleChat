...
def get_channel_members(channel_id: int, session: SessionDep) -> list[str]:
    statement = select(UserChannels.username).where(UserChannels.channel_id == channel_id)
...
def get_channel_by_id(channel_id: int, session: SessionDep) -> Channel | None:
...
def get_channels_by_name(channel_name: str, session: SessionDep) -> Sequence[_T] | list[Channel]:
    ...
    """
    try:
        statement: Select = select(Channel).where(Channel.channel_name.ilike(f"%{channel_name}%"))
        channels = session.exec(statement).all()
        print(channels)
        return channels
    except Exception as e:
...
def create_channel(channel_name: str, description: str | None, session: SessionDep) -> Channel:
    if not channel_name:
    ...
    try:
        ...
        session.rollback()
        raise ValueError(f"A channel named '{channel_name}' already exists")
    except Exception as e:
        raise Exception(e)
...
def leave_channel(channel_id: int, username: str, session: Session) -> None:
    if not channel_id or not username:
        raise Exception("channel_id and username are required")
