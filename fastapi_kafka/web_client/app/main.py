...
CHANNEL_MANAGER_URL = os.getenv("CHANNEL_MANAGER_URL", "http://channel_manager")
...
async def register_page(request: Request, session_id: Annotated[Optional[str], Cookie()] = None):
    if session_id and session_id in active_sessions:
        return RedirectResponse(url="/channels")
    return templates.TemplateResponse("register.html", {"request": request, "error_message": ""})
...
async def register(
    ...
    try:
        ...
        if reg_response.status_code != 200:
            ...
            return templates.TemplateResponse(
                "register.html",
                {"request": {"method": "POST"}, "error_message": error_detail},
        ...
        session_id = str(uuid.uuid4())
        active_sessions[session_id] = {"username": username, "token": access_token}
...
@app.get("/chat/{channel_slug}", response_class=HTMLResponse)
async def chat(
    request: Request, channel_slug: str, user_data: dict = Depends(get_current_user)
):
    """Render the chat page for a specific channel"""
    search_term = channel_slug.replace("_", " ")
    channel_id = None
    channel_name = channel_slug
    try:
        response = requests.get(f"{CHANNEL_MANAGER_URL}/channels/{search_term}")
        if response.status_code == 200:
            for ch in response.json():
                if to_slug(ch["channel_name"]) == channel_slug:
                    channel_id = ch["id"]
